...
class EditMealScreen extends StatefulWidget {
  ...
  const EditMealScreen({super.key, required this.mealEntity});
...
class _EditMealScreenState extends State<EditMealScreen> {
  ...
  _loadData() {
    _mealNameController.text = widget.mealEntity.name;
    _foodItems.addAll(widget.mealEntity.foods);
    setState(() {});
  }
  ...
  void _onUpdateMeal() {
    ...
    final newMeal = widget.mealEntity.copyWith(
      name: mealName,
      foods: List.from(_foodItems),
    );

    entryViewModel.updateMeal(meal: newMeal);
    snack.success(text: '$mealName updated');
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Edit ${widget.mealEntity.name}')),
      body: Padding(
            ...
            if (_foodItems.isNotEmpty)
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  onPressed: viewModel.isLoading ? null : _onUpdateMeal,
                  ...
                  label: Text('Update', style: AppTextStyle.button),
                ),
              ),
          ],
        );
